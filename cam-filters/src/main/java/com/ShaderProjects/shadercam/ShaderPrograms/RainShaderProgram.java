...
public class RainShaderProgram extends Shader {
    ...
    public RainShaderProgram(Context context, VideoRenderer renderer) {
    ...
    public void draw() {
        ...
        double RAIN_CHANCE = 0.1;
        if(Math.random() > RAIN_CHANCE) {
            setTouch((float)Math.random() * renderer.getWidth(), (float)Math.random() * renderer.getHeight());
        }
        ...
        rippleShader.draw(fboCurrent.texture, fboOld.texture, -1, -1);
        swapBuffers();
